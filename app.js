...
(function () {
  ...
  const SCALES = [
    ...
    /* PSIS 运动心理技能量表（青少年简版，18 题；基于 Mahoney PSIS 框架，6 维度） */
    {
      id: 'psis', name: '运动心理技能量表（PSIS 简版）', short: 'PSIS', group: '运动员心理选拔',
      intro: '以下描述了一些与运动表现有关的心理特点，请选择符合自己的程度（1=完全不符合，5=完全符合）。带（反向）的题请按相反方向理解。',
      options: [{ v: 1, t: '完全不符合' }, { v: 2, t: '不符合' }, { v: 3, t: '有点符合' }, { v: 4, t: '基本符合' }, { v: 5, t: '完全符合' }],
      items: [
        '我非常想在运动中表现出色',
        '我努力训练，希望跻身本项目顶尖水平',
        '我渴望在运动中取得成功',
        '大多数比赛我都满怀信心，相信自己会发挥好',
        '即使某次发挥较差，我通常也能保持自信',
        '我对自己有信心',
        '在即将开始比赛前的最后几分钟，我常感到惊慌（反向）',
        '比赛前我会担心自己能否比好（反向）',
        '重要比赛前我感到极度焦虑（反向）',
        '比赛前我常在脑海里“演练”自己的动作',
        '我在脑中练习时，能像看录像带一样“看见”自己的表现',
        '我会通过在脑中建立表征来为比赛做准备',
        '我认为团队精神非常重要',
        '无论我个人表现多好，球队输了我也感到难过',
        '如果队友不尽全力，我会生气',
        '比赛中我常难以集中注意力（反向）',
        '比赛开始时，我难以抛开之前在做的事（反向）',
        '比赛中我容易被旁人分散注意（反向）'
      ],
      compute(a) {
        const g = (n) => (+a[n - 1] || 0);
        const rv = (n) => (6 - g(n));
        const MT = g(1) + g(2) + g(3);
        const SC = g(4) + g(5) + g(6);
        const AC = rv(7) + rv(8) + rv(9);
        const MP = g(10) + g(11) + g(12);
        const TE = g(13) + g(14) + g(15);
        const C = rv(16) + rv(17) + rv(18);
        const rows = [
          { label: '动机 MT', value: MT, of: 15 },
          { label: '自信 SC', value: SC, of: 15 },
          { label: '焦虑控制 AC', value: AC, of: 15 },
          { label: '心理准备 MP', value: MP, of: 15 },
          { label: '团队重要性 TE', value: TE, of: 15 },
          { label: '注意集中 C', value: C, of: 15 }
        ];
        const low = [MT, SC, AC, MP, TE, C].filter(v => v < 9).length;
        const level = low === 0 ? 'normal' : 'warn';
        const levelText = low === 0 ? '六项心理技能较均衡' : `${low} 项偏弱`;
        const note = '每维度 3–15 分，越高越好。射击/射箭尤其看重「焦虑控制 AC」「注意集中 C」「自信 SC」三项。';
        return { level, levelText, sections: [{ title: 'PSIS 六维度（各 3–15 分）', rows }], notes: [note] };
      }
    },
